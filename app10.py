...
SETTINGS_BOTTOM_GAP = "2rem"      # 設定區塊與最底部的安全距離
...
def connect_google_sheets(url):
    ...
    try:
        ...
        # 優先方案：嘗試從 Streamlit 雲端 Secrets 讀取
        try:
            if "type" in st.secrets:
                creds_dict = dict(st.secrets)
                creds = ServiceAccountCredentials.from_json_keyfile_dict(creds_dict, scope)
        except Exception:
            pass # 雲端沒設定，忽略錯誤繼續找本地

        # 備用方案：從電腦本地資料夾讀取 google_key.json
        if not creds:
