...
<style>
    ...
    .main-title-box {
        ...
        padding: 12px 15px; border-radius: 6px; text-align: center; margin-bottom: 20px; 
        box-shadow: 0 2px 5px rgba(0,0,0,0.02);
    }
    .main-title-text { color: #4A4238; margin: 0; padding: 0; font-weight: bold; font-size: 22px; letter-spacing: 1px; }
    .sub-title-text { color: #4A4238; font-weight: bold; font-size: 16px; margin-bottom: 5px; }
    
    /* ✨ 優化：強制縮小下載按鈕下方的留白，讓它跟上傳區塊更緊密 */
    .stDownloadButton { margin-bottom: -15px !important; }
</style>
...
if doc:
    ...
    # ==========================================
        ...
        """, unsafe_allow_html=True)
        st.write("上傳**空白底圖**，系統會幫您完美壓印最新日期與折扣碼！支援無限多個版位。")
        ...
        if "active_slots" not in st.session_state:
            st.session_state.active_slots = sorted(existing_ids) if existing_ids else [1, 2]
            
        c_add, c_space = st.columns([2, 5])
        with c_add:
            if st.button("➕ 新增一個全新版位", use_container_width=True):
                # ✨ 智慧尋找「最小可用空缺」來替補
                next_id = 1
                while next_id in st.session_state.active_slots:
                    next_id += 1
                st.session_state.active_slots.append(next_id)
                st.session_state.active_slots.sort() # 確保版面永遠按數字順序排列
                st.rerun()
        ...
        for slot_id in st.session_state.active_slots:
            ...
            with current_col:
                ...
                with c_del:
                    if st.button("🗑️ 刪除", key=f"del_{slot_id}", help="刪除此版位"):
                        row_to_del = next((i + 1 for i, r in enumerate(cfg_data) if r and r[0] == f"coupon_{slot_id}"), None)
                        if row_to_del:
                            with st.spinner("刪除中..."):
                                ws_cfg.delete_rows(row_to_del)
                ...
                if existing_row and len(existing_row) > 1:
                    try:
                        ...
                        curr_img = base64_chunks_to_img(chunks)
                        st.image(curr_img, width=300)
                        st.caption("💡 **手機版：請直接「長按上方圖片」即可儲存。**")
                        
                        buf = BytesIO()
                        curr_img.save(buf, format="JPEG", quality=100)
                        st.download_button(label=f"💻 電腦版下載", data=buf.getvalue(), file_name=f"BearJoy_Coupon_{slot_id}.jpg", mime="image/jpeg", key=f"dl_{slot_id}")
                    except Exception: st.warning("圖片載入異常，請重新上傳")
                else:
                    st.info("此版位目前為空")
                
                # ✨ 優化：已移除中間的 <br> 讓間距更緊湊
                new_file = st.file_uploader(f"更換版位 {slot_id} 圖片", type=["png", "jpg", "jpeg"], key=f"up_{slot_id}", label_visibility="collapsed")
                
                if new_file:
                    with st.expander(f"✨ 開啟壓印控制台 (版位 {slot_id})", expanded=True):
                        img_to_process = Image.open(new_file)
                        
                        enable_text = st.checkbox("✒️ 啟動文字壓印", value=True, key=f"en_txt_{slot_id}")
                        final_img_to_save = img_to_process 
                        
                        if enable_text:
                            text_input = st.text_area("輸入文字 (可換行)", value="折扣碼: BEIBHD20\n(請留意效期至 6/30)", key=f"txt_{slot_id}")
                            
                            c_s, c_c, c_r = st.columns([1.2, 1, 1.2])
                            font_size = c_s.slider("📐 字體大小", 10, 200, 65, key=f"sz_{slot_id}")
                            text_color = c_c.color_picker("🎨 顏色", "#FFFFFF", key=f"col_{slot_id}")
                            rotation_angle = c_r.slider("🔄 旋轉", -180, 180, 0, key=f"rot_{slot_id}")
                            
                            c_x, c_y = st.columns(2)
                            x_pos = c_x.slider("↔️ 左右 (X)", 0, img_to_process.width, img_to_process.width//2, key=f"x_{slot_id}")
                            y_pos = c_y.slider("↕️ 上下 (Y)", 0, img_to_process.height, int(img_to_process.height*0.7), key=f"y_{slot_id}")
                            
                            preview_img = img_to_process.copy().convert("RGBA")
                        ...
                        if st.button(f"✅ 確認儲存", type="primary", key=f"btn_{slot_id}"):
                ...
                st.markdown("<hr style='border: 1px dashed #E3DFD5;'>", unsafe_allow_html=True)
