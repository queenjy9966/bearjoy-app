...
# ✨ 企業級資安：優先從 Secrets 讀取，不寫死在程式碼中
has_secrets = "gemini_api_key" in st.secrets and "google_sheet_url" in st.secrets
api_key = ""
sheet_url = ""
...
<style>
    ...
    /* ========================================================= */
    /* ✨ 暴力破解手機斷行：強制下載鈕與操控按鈕「水平同一行」 */
    /* ========================================================= */
    @media (max-width: 9999px) {
        .element-container:has(.ctrl-row) + .element-container > div > div[data-testid="stHorizontalBlock"] {
        ...
        .element-container:has(.ctrl-row) + .element-container > div > div[data-testid="stHorizontalBlock"] > div[data-testid="column"]:nth-child(1) {
            flex: 1 1 auto !important; /* 下載按鈕佔滿剩餘空間 */
            width: 100% !important;
            padding: 0 !important;
        }
        /* 後面三個按鈕強制鎖死 34px，絕對不准換行或變形 */
        .element-container:has(.ctrl-row) + .element-container > div > div[data-testid="stHorizontalBlock"] > div[data-testid="column"]:nth-child(n+2) {
    ...
    .stDownloadButton { margin: 0 !important; width: 100% !important; }
    .stDownloadButton button { height: 34px !important; min-height: 34px !important; padding: 0 10px !important; width: 100% !important; margin: 0 !important; font-size: 14px !important;}
    ...
    .ctrl-btn-style button p, .ctrl-btn-style button div, .ctrl-btn-style button span {
        margin: 0 !important; padding: 0 !important; line-height: 1 !important; display: flex !important; justify-content: center !important; align-items: center !important; font-size: 16px !important;
    }

    .ctrl-btn-style button:hover { background: #E3DFD5 !important; color: #4A4238 !important; }
    .btn-del button { color: #A94442 !important; border: 1px solid #E6C5A8 !important; }
    .btn-del button:hover { background: #F8E3D0 !important; }

    /* ========================================================= */
    /* 文字方框與顏色調整紐，絕對強制「水平同一行」 */
    /* ========================================================= */
    @media (max-width: 9999px) {
        .element-container:has(.txt-col-row) + .element-container > div > div[data-testid="stHorizontalBlock"] {
            display: flex !important;
            flex-direction: row !important;
            flex-wrap: nowrap !important;
        ...
        .element-container:has(.txt-col-row) + .element-container > div > div[data-testid="stHorizontalBlock"] > div[data-testid="column"]:first-child {
            flex: 1 1 100% !important; 
            width: 100% !important;
            padding: 0 !important;
        }
        .element-container:has(.txt-col-row) + .element-container > div > div[data-testid="stHorizontalBlock"] > div[data-testid="column"]:last-child {
    ...
    div[data-testid="stColorPicker"] { margin-top: 0px !important; display: flex; flex-direction: column; justify-content: flex-end; }
...
with st.sidebar:
    ...
    # 🛡️ 資安魔法：如果在雲端抓到 Secrets，輸入框與眼睛按鈕「徹底消失」！
    if has_secrets and doc:
        st.markdown("""
        ...
        # 只有在沒有 Secrets 的情況下（如本機測試），才會顯示輸入框
        with st.expander("⚙️ 設定 (未偵測到雲端金鑰)", expanded=True):
        ...
        if err:
            st.markdown(f"<p style='color:#A94442; font-weight:bold; font-size:14px; text-align:center;'>⚠️ 連線失敗</p>", unsafe_allow_html=True)
...
if doc:
    ...
    # ==========================================
        ...
        for slot_id in st.session_state.active_slots:
            ...
            with current_col:
                ...
                # ✨ 魔法 1：下載鈕與操控按鈕，絕對強制「水平同一行」
                st.markdown('<span class="ctrl-row"></span>', unsafe_allow_html=True)
                c_dl, c_up, c_down, c_del = st.columns([1, 0.1, 0.1, 0.1]) 
                with c_dl:
                    if base_img:
                        buf = BytesIO()
                        # ✨ 畫質升級：使用無損 PNG 格式下載，保證清晰
                ...
                if base_img:
                    with st.expander(f"✨ 開啟壓印控制台", expanded=bool(new_file)):
                        ...
                        if enable_text:
                            st.markdown('<span class="txt-col-row"></span>', unsafe_allow_html=True)
                            c_txt, c_col = st.columns([7, 1]) 
                            with c_txt:
    ...
    st.warning("⚠️ 系統尚未連線，請檢查程式碼中的金鑰設定。")