...
<style>
    ...
    .element-container:has(.ctrl-row) + .element-container > div > div[data-testid="stHorizontalBlock"] > div[data-testid="column"]:first-child {
        flex: 1 1 auto !important;
        padding-right: 5px !important;
        min-width: 0 !important;
    }
    .element-container:has(.ctrl-row) + .element-container > div > div[data-testid="stHorizontalBlock"] > div[data-testid="column"]:not(:first-child) {
    ...
    .stDownloadButton { margin: 0 !important; }
    .stDownloadButton button { height: 28px !important; min-height: 28px !important; padding: 0 10px !important; width: 100% !important; margin: 0 !important; }
    ...
    .ctrl-btn-style button p, .ctrl-btn-style button div, .ctrl-btn-style button span {
        margin: 0 !important; padding: 0 !important; line-height: 26px !important; display: block !important; text-align: center !important;
    }
...
with st.sidebar:
    ...
    is_connected = bool(default_api and default_url)
    with st.expander("⚙️ 設定", expanded=not is_connected):
        ...
        if doc:
            st.markdown("<p style='color:#798571; font-weight:bold; font-size:14px; text-align:center;'>✅ 已連線</p>", unsafe_allow_html=True)
        else:
            st.markdown("<p style='color:#A94442; font-weight:bold; font-size:14px; text-align:center;'>⚠️ 尚未連線</p>", unsafe_allow_html=True)
    ...
    # ==========================================
        ...
        for slot_id in st.session_state.active_slots:
            ...
            with current_col:
                ...
                c_dl, c_up, c_down, c_del = st.columns([10, 1, 1, 1]) 
                with c_dl:
                    if base_img:
                        buf = BytesIO()
                        base_img.save(buf, format="JPEG", quality=100)
                        st.download_button(label=f"💻 下載", data=buf.getvalue(), file_name=f"BearJoy_Coupon_{display_num}.jpg", mime="image/jpeg", key=f"dl_btn_{slot_id}")
                    else:
                ...
                with c_del:
                    ...
                    if st.button("✕", key=f"del_btn_{slot_id}", help="刪除"):
