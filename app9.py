...
<style>
    ...
    .sub-title-text { color: #4A4238; font-weight: bold; font-size: 15px; margin: 0; white-space: nowrap; }
    
    /* ✨ 修正：取消過度擠壓的負邊距，讓畫面呼吸 */
    .stDownloadButton { margin-top: 5px !important; }
    div[data-testid="stExpander"] { margin-bottom: 10px !important; }
    
    /* 鎖定標題、排序與刪除按鈕，強制在一行不換行 */
    .element-container:has(.title-del-row) + .element-container div[data-testid="stHorizontalBlock"] {
        display: flex !important;
        flex-wrap: nowrap !important;
        align-items: center !important;
        margin-bottom: 5px !important;
    }
    .element-container:has(.title-del-row) + .element-container div[data-testid="stHorizontalBlock"] > div[data-testid="column"] {
        min-width: 0 !important; width: auto !important; padding: 0 3px !important;
    }
    .element-container:has(.title-del-row) + .element-container div[data-testid="stHorizontalBlock"] > div[data-testid="column"]:first-child {
        flex: 1 1 auto !important; 
    }
    .element-container:has(.title-del-row) + .element-container div[data-testid="stHorizontalBlock"] > div[data-testid="column"]:not(:first-child) {
        flex: 0 0 auto !important; margin-top: 0px !important;
    }
    
    /* 新版控制按鈕樣式 (變成小正方形) */
    .ctrl-btn-style button {
        padding: 0px !important; width: 32px !important; min-height: 32px !important; height: 32px !important;
        font-size: 14px !important; font-weight: bold !important; color: #4A4238 !important; border: 1px solid #D0CCC1 !important;
        display: flex; justify-content: center; align-items: center;
    }
    .btn-move button { background-color: #E3DFD5 !important; }
    .btn-move button:hover { background-color: #D0CCC1 !important; }
    .btn-del button { background-color: #E6C5A8 !important; border: 1px solid #D9B391 !important; color: #A94442 !important;}
    .btn-del button:hover { background-color: #D9B391 !important; }

    /* ✨ 修正：控制台恢復合理間距，文字不重疊 */
    div[data-testid="stSlider"], div[data-testid="stColorPicker"] { margin-bottom: 0px !important; padding-bottom: 0px !important; }
    div[data-testid="stSlider"] label, div[data-testid="stColorPicker"] label { font-size: 13px !important; margin-bottom: 5px !important; }
    div[data-testid="stTextArea"] textarea { min-height: 60px !important; height: 60px !important; }
</style>
    ...
    # ==========================================
        ...
        for slot_id in st.session_state.active_slots:
            ...
            with current_col:
                ...
                # ✨ 極簡化刪除按鈕
                c_title, c_up, c_down, c_del = st.columns([5, 1, 1, 1])
                with c_title:
                    st.markdown(f"<p class='sub-title-text' style='margin-top:6px;'>🎟️ 折價券版位 {slot_id}</p>", unsafe_allow_html=True)
                with c_up:
                ...
                with c_del:
                    ...
                    if st.button("🗑️", key=f"del_btn_{slot_id}", help="刪除"):
                ...
                if base_img:
                    with st.expander(f"✨ 開啟壓印控制台 (版位 {slot_id})", expanded=bool(new_file)):
                        ...
                        if enable_text:
                            ...
                            font_size = c1.slider("📐 大小", 10, 200, 65, key=f"sz_{slot_id}")
                            text_color = c2.color_picker("🎨 顏色", "#FFFFFF", key=f"col_{slot_id}")
                            rotation_angle = c3.slider("🔄 旋轉", -180, 180, 0, key=f"rot_{slot_id}")
                            
                            cx, cy = st.columns(2)
                            x_pos = cx.slider("↔️ 左右", 0, base_img.width, base_img.width//2, key=f"x_{slot_id}")
                            y_pos = cy.slider("↕️ 上下", 0, base_img.height, int(base_img.height*0.7), key=f"y_{slot_id}")
