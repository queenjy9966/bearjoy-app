...
<style>
    ...
    .sub-title-text { 
        color: #4A4238; font-weight: bold; font-size: 16px; margin: 0; 
    }
    
    /* 縮小間距，確保不重疊 */
    div[data-testid="stFileUploader"] { margin-bottom: -10px !important; margin-top: 5px !important; }
    ...
    .element-container:has(.ctrl-row) + .element-container > div > div[data-testid="stHorizontalBlock"] > div[data-testid="column"] {
    ...
    .ctrl-btn-style button:hover { background: #F0EDE5 !important; color: #4A4238 !important; }
    .btn-del button { color: #A94442 !important; border: 1px solid #E6C5A8 !important; font-size: 14px !important; }
    ...
    div[data-testid="stSlider"] > label {
        flex: 0 0 auto !important; width: auto !important; margin-bottom: 0px !important;
    ...
    div[data-testid="stTextArea"] label, div[data-testid="stColorPicker"] label {
        ...
        margin-bottom: 2px !important; min-height: 20px !important;
    }
    div[data-testid="stTextArea"] textarea { min-height: 60px !important; height: 60px !important; padding: 6px !important; }
    div[data-testid="stColorPicker"] { margin-top: 0px !important; }
    div[data-testid="stColorPicker"] > div { width: 100% !important; }
    div[data-testid="stColorPicker"] div[role="button"] { width: 100% !important; border-radius: 6px !important; height: 60px !important; }
</style>
...
if doc:
    ...
    # ==========================================
        ...
        with c_add:
            if st.button("➕ 新增一個全新版位", type="primary"):
                next_id = 1
        ...
        default_coupon_txt = f"折扣碼: BEIBHD20\n(請留意效期至 {now.month}/{last_day})"
        ...
        for slot_id in st.session_state.active_slots:
            ...
            display_num = display_idx + 1 # ✨ 永遠固定的視覺名稱 (版位 1, 2, 3...)
            ...
            with current_col:
                ...
                # ✨ 神奇控制列：下載鈕 + 排序/刪除鈕完美並排
                st.markdown('<span class="ctrl-row"></span>', unsafe_allow_html=True)
                c_dl, c_up, c_down, c_del = st.columns([5, 1.2, 1.2, 1.2])
                with c_dl:
                    if base_img:
                        buf = BytesIO()
                        base_img.save(buf, format="JPEG", quality=100)
                        st.download_button(label=f"💻 電腦版下載", data=buf.getvalue(), file_name=f"BearJoy_Coupon_{display_num}.jpg", mime="image/jpeg", key=f"dl_btn_{slot_id}")
                    else:
                ...
                # 上傳區塊
                new_file = st.file_uploader(f"更換版位 {display_num} 圖片", type=["png", "jpg", "jpeg"], key=f"up_file_{slot_id}")
                ...
                if base_img:
                    with st.expander(f"✨ 開啟壓印控制台", expanded=bool(new_file)):
                        if not new_file:
                            st.caption("⚠️ **溫馨提示：** 目前使用的是舊圖，建議上傳「乾淨空白底圖」再重新壓印字體。")
                        ...
                        if enable_text:
                            # ✨ 文字方框與顏色並排
                            c_txt, c_col = st.columns([3, 1])
                            with c_txt:
                                text_input = st.text_area("✍️ 文字", value=default_coupon_txt, key=f"txt_{slot_id}")
                            with c_col:
                        ...
                        if st.button(f"✅ 確認儲存", type="primary", key=f"btn_save_{slot_id}"):
