...
# ==========================================
# 2. BearJoy 視覺佈局 (高度自訂與完美鎖定版)
# ==========================================
st.set_page_config(page_title="BearJoy 智能客服", page_icon="✦", layout="wide")

# 💡【專屬高度控制台】您可以在這裡自由調整上下位置！
NAV_TOP_POSITION = "0rem"         # 導航距離頂部的空白高度 (預設 1rem，數值越小越貼頂，如 "0.5rem" 或 "0rem")
SETTINGS_BOTTOM_POSITION = "40px" # 設定區塊距離底部的懸空高度 (預設 40px，數值越大越往上移)
...
<style>
    ...
    /* ========================================= */
    /* 側邊欄精準鎖定策略 (導航死釘頂部、徹底無滑軌) */
    /* ========================================= */
    [data-testid="stSidebar"] { 
        background-color: #F0EDE5 !important; 
        border-right: 1px solid #E3DFD5 !important; 
    }
    
    [data-testid="stSidebarUserContent"] { 
        display: flex !important;
        flex-direction: column !important;
        justify-content: flex-start !important; /* 強制內容從最上方開始，永不移動 */
        height: 100vh !important;
        position: relative !important; 
        padding-top: """ + NAV_TOP_POSITION + """ !important; /* 套用您的專屬高度參數 */
        padding-bottom: 1.5rem !important;
        overflow: hidden !important; /* 徹底封印滑軌，保證導航永不移動 */
    }

    /* 將最後一個隱形元素推到下面，確保彈性空間正常 */
    ...
    [data-testid="stSidebar"] [data-testid="stExpander"] {
        ...
        bottom: """ + SETTINGS_BOTTOM_POSITION + """ !important; /* 套用您的專屬高度參數 */
        left: 15px !important;
...
with st.sidebar:
    ...
    # CSS 已經將 stExpander 絕對定位到底部
    with st.expander("⚙️ 設定", expanded=False):
        api_key = st.text_input("API 金鑰:", value=saved_api, type="password")
        sheet_url = st.text_input("試算表網址:", value=saved_url)
        
        if st.button("儲存連線", use_container_width=True):
            save_config(api_key, sheet_url)
            st.rerun()
            
        doc = connect_google_sheets(sheet_url) if sheet_url else None
        
        # 使用自訂 HTML 完全掌控寬高與字體，確保與按鈕 100% 一致 (無圖示)
        if doc: 
            ...
            if start_btn and files and api_key:
                ...
                for file in files:
                    ...
                    with st.spinner(f"🏃‍♀️ AI 正在為您撰寫 {file.name} 的專屬回覆..."):
                        ...
                        # ==========================================
                        # 🚦 防塞車安全閥：每張圖片處理完強制休息 6 秒
                        # ==========================================
                        time.sleep(6)

                if doc and results_to_cloud:
                    try:
                        ws_history = doc.worksheet("回覆紀錄")
    ...
    with tab2:
        ...
        if doc:
            try:
                ...
                st.error("讀取失敗：請確保試算表『VIP名單』的第一列沒有重複的空白欄位。")
        else:
