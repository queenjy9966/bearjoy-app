...
if doc:
    if menu == "智能客服系統":
        ...
        st.markdown("## ✦ 每月折價券管理")
        st.write("上傳的圖片將自動壓縮並同步至雲端，確保手機隨時可快速下載發送給客人。")
            ...
            with col:
                ...
                if existing and existing.get('內容'):
                    try:
                        ...
                        st.download_button(label=f"📥 下載折價券 {i}", data=buf.getvalue(), file_name=f"BearJoy_Coupon_{i}.jpg", mime="image/jpeg", key=f"dl_{i}")
                    except Exception as e:
